...
void TableSchema::setPrimaryKey(std::optional<std::string> primaryKey) {
  if (primaryKey) {
    if (indexByName_.find(*primaryKey) == indexByName_.end()) {
      throw std::invalid_argument("Primary key column not found in schema: " + *primaryKey);
    }
  }
...
bool SchemaValidator::checkConstraints(const Column& col, const Value& v, std::string& err) {
  // Type-specific richer constraints
  switch (col.type) {
    case ColumnType::String: {
      if (v.type() != ValueType::String) return true; // type mismatch handled elsewhere
    ...
    case ColumnType::Float: {
      double d = 0.0;
      if (v.type() == ValueType::Integer) d = static_cast<const IntegerValue&>(v).asFloat();
      else if (v.type() == ValueType::Float) d = static_cast<const FloatValue&>(v).asFloat();
      else return true; // type mismatch handled elsewhere
...
std::string SchemaValidator::validateDocument(const DocumentSchema& schema, const Document& doc) {
...
std::string SchemaValidator::validateUnique(const TableSchema& schema, const std::vector<Row>& rows,
                                            bool ignoreNulls) {
  std::vector<size_t> uniqueIdx;
  uniqueIdx.reserve(schema.columns().size());
  for (size_t i = 0; i < schema.columns().size(); ++i) {
    if (schema.columns()[i].unique) uniqueIdx.push_back(i);
  }
  if (uniqueIdx.empty()) return {};
  ...
  for (size_t r = 0; r < rows.size(); ++r) {
    ...
    for (size_t ui = 0; ui < uniqueIdx.size(); ++ui) {
      ...
      if (!valPtr) {
        ...
        if (!inserted) {
          return "Duplicate value for unique column '" + schema.columns()[idx].name + "'";
        }
        continue;
      }
...
std::string SchemaValidator::validateUnique(const DocumentSchema& schema, const std::vector<Document>& docs,
                                            bool ignoreNulls) {
  std::vector<std::string> uniqueFields;
  uniqueFields.reserve(schema.fields().size());
  for (const auto& kv : schema.fields()) {
    if (kv.second.unique) uniqueFields.push_back(kv.first);
  }
  if (uniqueFields.empty()) return {};

  std::vector<std::unordered_map<std::string, size_t>> seen(uniqueFields.size());
  for (size_t di = 0; di < docs.size(); ++di) {
    ...
    for (size_t ui = 0; ui < uniqueFields.size(); ++ui) {
      ...
      bool isNullish = (it == doc.end()) || (!it->second) || (it->second && it->second->type() == ValueType::Null);
      if (isNullish) {
        if (ignoreNulls) continue;
        auto [sit, inserted] = seen[ui].emplace("<null>", di);
        if (!inserted) return "Duplicate value for unique field '" + fname + "'";
        continue;
      }
