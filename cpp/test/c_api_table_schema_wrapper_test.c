...
static KDB_Value V_NULL() { KDB_Value v; v.type = KDB_VAL_NULL; return v; }
static KDB_Value V_INT(long long x) { KDB_Value v; v.type = KDB_VAL_INTEGER; v.as.i64 = x; return v; }
static KDB_Value V_STR(const char* s) { KDB_Value v; v.type = KDB_VAL_STRING; v.as.str = s; return v; }
...
int main() {
  ...
  KDB_ColumnConstraints name_cc = { .min_len = 2, .max_len = 10, .one_of = NULL, .one_of_count = 0, .min_value = NAN, .max_value = NAN };
  ...
  assert(KadeDB_TableSchema_ValidateUniqueRows(s, rows_ignore, 2, 1, buf, sizeof(buf)) == 1);
  assert(KadeDB_TableSchema_ValidateUniqueRows(s, rows_ignore, 2, 0, buf, sizeof(buf)) == 0);
