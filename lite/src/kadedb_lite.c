...
kadedb_lite_t* kadedb_lite_open(const char* path) {
  ...
  if (!h->options) { free(h); return NULL; }
  ...
  if (!h->ropts || !h->wopts) {
    if (h->ropts) rocksdb_readoptions_destroy(h->ropts);
    if (h->wopts) rocksdb_writeoptions_destroy(h->wopts);
    rocksdb_close(h->db);
    rocksdb_options_destroy(h->options);
    free(h);
    return NULL;
...
#ifdef KADEDB_LITE_HAS_ROCKSDB
  if (!db || !db->db || !key || !value) return -1;
  char* err = NULL;
...
int kadedb_lite_get(kadedb_lite_t* db, const char* key, char** value_out, size_t* value_len_out) {
#ifdef KADEDB_LITE_HAS_ROCKSDB
  if (!db || !db->db || !key || !value_out) return -1;
  char* err = NULL;
  size_t outlen = 0;
  ...
  if (!buf) { rocksdb_free(val); return -1; }
  memcpy(buf, val, outlen);
  buf[outlen] = '\0';
  rocksdb_free(val);
  *value_out = buf;
  if (value_len_out) *value_len_out = outlen;
  return 0;
#else
  (void)db; (void)key;
  const char* msg = "stub";
